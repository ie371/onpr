export default {
	...
	actions: {
		change_pr_ot(context, p) {
			context.commit('mu_pr_ot', p);
		},
		change_pr_gvs(context, p) {
			context.commit('mu_pr_gvs', p);
		},
		actnum(context, q) {
		...
		act_ot_null(context, dtr) {
			context.commit('mu_ot_null', dtr);
		},
		act_gvs_null(context, dtr) {
			context.commit('mu_gvs_null', dtr);
		},
		actrescalc(context, ek) {
			context.commit('murescalc', ek);
		},
		actPodp(context, s) {
			context.commit('muPodp', s);
		},
		actOKP(context, s) {
			context.commit('muOKP', s);
		},
		actGVSto(context, s) {
			context.commit('muGVSto', s);
		},
		// actGVSNO(context){ context.commit('muGVSNO') },
		change_fuCo(context, p) {
			context.commit('mufuCo', p);
		},
		change_tip_im4(context, p) {
			context.commit('mutip_im4', p);
		}
	},
	...
	mutations: {
		...
		mutipkp(state, payload) {
			state.isxcalc.tipkp = payload;
		},
		...
		muOKP(state, payload) {
			state.isxcalc.onlineKP = payload;
		},
		mufuCo(state, payload) {
			state.isxcalc.fuCo = payload;
		},
		mutip_im4(state, payload) {
			state.isxcalc.tipIMg4 = payload;
		}
	}
};
